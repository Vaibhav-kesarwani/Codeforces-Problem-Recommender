...
let problems_div = document.getElementById("problems");
let rating = document.getElementById("rank_display");
let ptags = [];
let levels = [];
let handle = 'none';
let estimated_rating = 0;
// Vars for charts
let tags = {};
...
function RecommendProb() {
    ...
    $.get(api_url + probsubmitted, { 'handle': handle }, function (data, status) {
        ...
        for (let i = 0; i < res.length; i++) {
            if (user_prob_set.includes(res[i].problem.contestId + "_" + res[i].problem.name))
                continue;
...
function tags_n_ratings(ptags, user_prob_set) {
    $.get(api_url + userinfo, { 'handles': handle })
        ...
        if (contest_list.length == 0) {
            if (rating)
                $(rating).css('color', 'black').text("NA");
            ...
            if (rating)
                $(rating).css('color', rating_color[curr_rank] || 'black').text(curr_rating);
            $('#max_rating_display').css('color', rating_color[max_rank] || 'black').text(maxRating);
            $('#max_rank_display').css('color', rating_color[max_rank] || 'black').text("(" + capitalize(max_rank) + ")");
            $('#current_rank_display').css('color', rating_color[curr_rank] || 'black').text(capitalize(curr_rank));
        }
...
function EMH(rating, usersubmits) {
    $.get(api_url + prob, { 'tags': "" })
        ...
        for (let i = 0; i < total_no_prob; i++) {
            if (!usersubmits.includes(pset[i].contestId + "_" + pset[i].name))
                not_attempted_prob.push(pset[i]);
        }
...
function drawCharts() {
    ...
    let tagOptions = {
        width: Math.max(600, (_a = $('#tags').width()) !== null && _a !== void 0 ? _a : 0),
        height: Math.max(600, (_b = $('#tags').width()) !== null && _b !== void 0 ? _b : 0) * 0.75,
    ...
    $('#display_values').click(function () {
        ...
        $.get(api_url + "user.rating", { 'handle': handle })
            ...
            if (contest_list.length == 0) {
                ...
                $('#recent_contests').text("");
            for (let i = 0; i < Math.min(5, contest_list.length); i++)
